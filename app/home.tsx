...
import { registerPushNotifications, sendQuickTestNotification, sendRemoteTestNotification, subscribeToPushTokenChanges } from '@/lib/notifications';
...
export default function HomeScreen() {
  ...
  const load = useCallback(async () => {
    ...
    if (error) {
      if (error.message.includes('NO_HOUSEHOLD')) router.replace('/setup');
      else Alert.alert('Could not load household', error.message);
    } else {
  ...
  useEffect(() => {
    ...
    registerPushNotifications(data.householdId)
      .then(() => setNotificationState('enabled'))
      .catch(() => setNotificationState('blocked'));
  ...
  async function testNotification() {
    try {
      ...
      Alert.alert(t('testSent'), 'You should receive an immediate local notification and a remote FCM push shortly.');
    } catch (error) {
